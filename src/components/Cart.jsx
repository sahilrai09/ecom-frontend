      ...
      try {
        ...
        const cartItemsWithImages = await Promise.all(
          updatedCartItems.map(async (item) => {
            try {
              const imgResp = await API.get(`/product/${item.id}/image`, {
                responseType: "blob",
              });
              const imageUrl = URL.createObjectURL(imgResp.data);
  ...
  const handleCheckout = async () => {
    try {
      for (const item of cartItems) {
        ...
        // no image update is required here; leave blank or attach if available
        if (cartImage && cartImage.length) cartProduct.append("imageFile", cartImage[0]);
        cartProduct.append(
          "product",
